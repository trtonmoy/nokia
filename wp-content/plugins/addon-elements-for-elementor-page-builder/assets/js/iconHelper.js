window.renderIconHtml = function (
  view,
  elementor,
  sett,
  control_name,
  ...
  let iconHtml = "";
  if (
    sett[control_name + "_graphic_type"] != "none" &&
    sett[control_name + "_graphic_type"] != ""
  ) {
    icon_class +=
      " eae-gbl-icon eae-graphic-type-" +
      sett[control_name + "_graphic_type"] +
      " elementor-animation-" +
    ...
    if (sett[control_name + "_graphic_type"] == "lottie") {
      if (
        sett[control_name + "_lottie_animation_url"] != "" ||
        sett[control_name + "_source_json"]["url"] != ""
      ) {
        icon_class += " eae-lottie-animation eae-lottie";
        lottie_data = {
          loop:
            sett[control_name + "_lottie_animation_loop"] === "yes"
              ? true
              : false,
          reverse:
            sett[control_name + "_lottie_animation_reverse"] === "yes"
              ? true
              : false,
        };
        if (
          sett[control_name + "_source"] == "media_file" &&
          sett[control_name + "_source_json"]["url"] != ""
        ) {
        ...
        view.addRenderAttribute(
          "panel-icon-" + index,
          "data-lottie-settings",
          JSON.stringify(lottie_data)
        );
      }
    }
    ...
    if (sett[control_name + "_graphic_type"] == "lottie") {
      if (lottie_data.url != undefined) {
        iconHtml = `<span ${view.getRenderAttributeString(
          "panel-icon-" + index
        )}></span>`;
      }
    } else {
      if (sett[control_name + "_graphic_type"] === "icon") {
        if (sett[control_name + "_graphic_icon"]["value"] != "") {
          var icon = elementor.helpers.renderIcon(
            view,
            sett[control_name + "_graphic_icon"],
            { "aria-hidden": true },
            "i",
            "object"
          );
